---
name: troubleshoot-inventory-tool
description: Use when assessing or diagnosing a support ticket involving Qubership Inventory Tool CLI (Inventory Tool CLI, Inventory Tool, ADIT, inventory-tool) or its components (startup and configuration, Git and the super repository, flow execution and progress, CI and Docker execution, graph dumps and obfuscation, Confluence export, or extensions and custom tasks), including installation, configuration, and runtime failures. Read-only and advisory; no live system access.
---
...
Diagnose one reported problem with the Qubership Inventory Tool CLI. Read the user's description and any attached
...
2. **Localize.** From the text and log signatures, name the **component** (startup and configuration; Git and the super
   repository; flow execution and progress; CI mode and the Docker image; graph dumps and obfuscation; Confluence
   export; extensions and custom tasks).
